                                    ...
                                    int full_width, int full_height)
{
    ...
    for (i = 0;  i < count;  i++)
    {
        const int currDiff = abs(images[i].full_width * images[i].full_height -
                        ...
                        int xhot, int yhot, GLFWbool icon)
{
    ...
    bi.bV5Width       = image->full_width;
    bi.bV5Height      = -image->full_height;
    bi.bV5Planes      = 1;
    bi.bV5BitCount    = 32;
    ...
    mask = CreateBitmap(image->full_width, image->full_height, 1, 1, NULL);
    if (!mask)
    {
        _glfwInputErrorWin32(GLFW_PLATFORM_ERROR,
    ...
    for (i = 0;  i < image->full_width * image->full_height;  i++)
    {
        target[0] = source[2];
        target[1] = source[1];
        target[2] = source[0];
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    if (window->monitor)
    {
        ...
        fullWidth  = mode.full_width;
        fullHeight = mode.full_height;
    }
    else
    {
        xpos = CW_USEDEFAULT;
        ...
        getFullWindowSize(style, exStyle,
                          wndconfig->full_width, wndconfig->full_height,
    ...
    if (!window->monitor)
    {
        RECT rect = { 0, 0, wndconfig->full_width, wndconfig->full_height };
...
void _glfwPlatformGetWindowSize(_GLFWwindow* window, int* full_width, int* full_height)
{
    RECT area;
    GetClientRect(window->win32.handle, &area);

    if (full_width)
        *full_width = area.right;
    if (full_height)
        *full_height = area.bottom;
}

void _glfwPlatformSetWindowSize(_GLFWwindow* window, int full_width, int full_height)
{
    if (window->monitor)
    {
        if (window->monitor->window == window)
...
void _glfwPlatformGetFramebufferSize(_GLFWwindow* window, int* full_width, int* full_height)
{
    _glfwPlatformGetWindowSize(window, full_width, full_height);
}
                                     ...
                                     int* right, int* bottom)
{
    ...
    _glfwPlatformGetWindowSize(window, &full_width, &full_height);
    SetRect(&rect, 0, 0, full_width, full_height);
    ...
    if (top)
        *top = -rect.top;
    if (right)
        *right = rect.right - full_width;
    if (bottom)
        *bottom = rect.bottom - full_height;
}
                                   ...
                                   int refreshRate)
{
    if (window->monitor == monitor)
    {
        ...
        {
            RECT rect = { xpos, ypos, xpos + full_width, ypos + full_height };
...
void _glfwPlatformPollEvents(void)
{
    ...
    if (window)
    {
        ...
        if (window->win32.lastCursorPosX != full_width / 2 ||
            window->win32.lastCursorPosY != full_height / 2)
        {
            _glfwPlatformSetCursorPos(window, full_width / 2, full_height / 2);
        }
    }
}
